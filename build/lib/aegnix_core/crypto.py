...
def derive_ed25519_pub(priv_raw: bytes) -> bytes:
    """
    Derive the Ed25519 public key from a raw private key.

    Why this matters:
      • AE developers only need privkey_b64.
      • Derive the pubkey internally.
      • ABI Service has the pubkey in the keyring.

    Input:
        priv_raw : bytes — 32-byte Ed25519 private key
